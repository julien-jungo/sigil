...
public class AuthController {
    ...
    return switch (tokenService.issue(user)) {
      case Result.Ok<String, ?> ok -> {
        ...
        yield ResponseEntity.ok(
            new LoginResponse(ok.value(), Instant.now().plusSeconds(expirySeconds)));
      }
      case Result.Err<?, AuthError> err ->
          ResponseEntity.internalServerError()
              .body(
                  ProblemDetail.forStatusAndDetail(
                      HttpStatus.INTERNAL_SERVER_ERROR, "Token issuance failed"));
    };
  }
  ...
  public ResponseEntity<?> introspect(@Valid @RequestBody IntrospectRequest request) {
    return switch (tokenService.introspect(request.token())) {
      ...
      case Result.Err<?, ?> err ->
          ResponseEntity.ok(new IntrospectResponse(false, null, null, null));
    };
  }
